...
if(playerSide == west) then {
	...
	if(__GETC__(life_coplevel) > 4) then{
		_Btn7 ctrlSetText "Zerstoeren";
		_Btn7 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_destroyVehicle;";
	}else{
		_Btn7 ctrlShow false;
	};
	
	_Btn8 ctrlSetText "ADAC Rufen";
	_Btn8 buttonSetAction "[life_vInact_curTarget] spawn jl_fnc_requestADAC;";
	...
	if(life_job == 99 && life_adac_onDuty) then {
		_Btn4 ctrlSetText "ADAC angekommen";
		
		_Btn4 buttonSetAction "[[cursorTarget],""jl_fnc_deleteADAC"",civilian,FALSE] spawn life_fnc_MP;";
	}else{
		_Btn4 ctrlSetText "ADAC Rufen";
		_Btn4 buttonSetAction "[life_vInact_curTarget] spawn jl_fnc_requestADAC;";
	};
