...
BTC_attach_cargo =
{
	...
	_chopper vehicleChat format ["%1 angehaengt", _name_cargo];
...
BTC_detach_cargo =
{
	...
	vehicle player vehicleChat format ["%1 abgehaengt", _name_cargo];
...
[] spawn
{
	...
	if (BTC_def_hud == 1) then {player addAction [("<t color=""#ED2744"">" + ("Hud An\Aus") + "</t>"),BTC_dir_action, [[],{if (BTC_Hud_Cond) then {BTC_Hud_Cond = false;} else {BTC_Hud_Cond = true;_hud = [] spawn BTC_fnc_hud;};}], -8, false, false, "", "(vehicle player) isKindOf ""Helicopter"" && driver (vehicle player) == player"];};
	if (BTC_def_pip == 1) then {player addAction [("<t color=""#ED2744"">" + ("Kamera An\Aus") + "</t>"),BTC_dir_action, [[],BTC_l_camera], -9, false, false, "", "typeOf (vehicle player) in BTC_l_def_veh_pip"];};
	player addAction [("<t color=""#ED2744"">" + ("Anhaengen") + "</t>"),BTC_dir_action, [[],BTC_attach_cargo], 9, true, false, "", "[] call BTC_lift_check"];
	player addAction [("<t color=""#ED2744"">" + ("Ablassen") + "</t>"),BTC_dir_action, [[],BTC_detach_cargo], -9, true, false, "", "BTC_lifted == 1"];
